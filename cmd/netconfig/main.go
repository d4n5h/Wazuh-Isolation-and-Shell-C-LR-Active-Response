...
func main() {
	...
	if err != nil {
		output("C-LR Netconfig", "error", "system", "", fmt.Sprintf("Input error: %v", err), dt+" C-LR Netconfig")
		os.Exit(1)
	}
	...
	if len(input.Parameters.ExtraArgs) > 0 {
		dev = input.Parameters.ExtraArgs[0]
	}
	...
	switch action {
	case "flush-dns":
		stdout, stderr = flushDNS()
	case "flush-arp":
		stdout, stderr = flushARP()
	case "reset-adapter":
		if dev == "" {
			stderr = "extra_args[0] must be adapter/interface name"
		} else {
			stdout, stderr = resetAdapter(dev)
		}
	default:
		stderr = "No action provided. Use: flush-dns, flush-arp, reset-adapter"
	}
